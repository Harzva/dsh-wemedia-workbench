...
export function ContentNavigation({ controller }: { controller: WorkbenchController }) {
  ...
  const main = <WorkbenchBoundary onClose={() => controller.requestClose()}><div className="wm-workbench wm-content-center-shell"><style>{workbenchStyles}{navigationStyles}{contentHomeStyles}</style>
    ...
    <Modal open={libraryDialogOpen && state.open} onClose={() => setLibraryDialogOpen(false)} title="选择内容" closeLabel="关闭内容选择" className="wm-workbench wm-library-dialog"><style>{workbenchStyles}{contentHomeStyles}</style><div ref={libraryDialogBody} className="wm-content-dialog-body" tabIndex={-1}><ContentLibrarySidebar controller={library} workbench={controller} onDraftBatch={(scope, refs) => { void controller.previewDraftBatch(scope, refs); }} onSelect={choose} onCreateArticle={state.snapshot?.settings.hasWriteRoot ? create : undefined} /></div></Modal>
  ...
  return <><span ref={anchor} hidden data-wemedia-layout-anchor="" />{surface && <>
    ...
    {createPortal(<><span ref={returnButton} className="wm-content-mobile-library"><Button size="sm" onClick={() => showMobileLibrary(false)}>返回内容</Button></span><ContentLibrarySidebar controller={library} workbench={controller} onDraftBatch={(scope, refs) => { void controller.previewDraftBatch(scope, refs); }} onSelect={choose} onCreateArticle={state.snapshot?.settings.hasWriteRoot ? create : undefined} /></>, surface.library)}
