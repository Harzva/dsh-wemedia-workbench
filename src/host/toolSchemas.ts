...
const draftBatchEntrySchema = object({ ...fields(["contentRef", "title", "code", "safeMessage"]), revisionDigest: required(nullable(text)), inputDigest: required(nullable(text)), status: required(enumeration(["pending", "skipped", "blocked", "running", "succeeded", "failed", "cancelled", "reconcile_required"])), jobId: required(nullable(text)), intentId: required(nullable(text)), targetRef: required(nullable(text)) });
export const draftBatchPreviewSchema = object({ schemaVersion: required({ type: "string", const: "wemedia.draft-batch-preview/v1" }), ...fields(["intentId", "generationId", "expiresAt"]), scope: required(enumeration(["selected", "pending"])), entries: required(array(draftBatchEntrySchema)), eligibleCount: required(integer) });
export const draftBatchSchema = object({ schemaVersion: required({ type: "string", const: "wemedia.draft-batch/v1" }), ...fields(["batchId", "generationId", "createdAt"]), status: required(enumeration(["running", "completed", "stopped"])), entries: required(array(draftBatchEntrySchema)) });
export const draftBatchListSchema = object({ schemaVersion: required({ type: "string", const: "wemedia.draft-batch-list/v1" }), batches: required(array(draftBatchSchema)) });
